...
See --> https://github.com/tesseract-ocr/tesseract/wiki/_improve_quality
"""
...
def remove_shadow(image):
    ...
    for plane in rgb_planes:
        ...
        bg_img = cv.median_blur(dilated_img, 11)
...
def detect_rotation(image):
    ...
    img_gray = cv.cvt_color(image, cv.COLOR_BGR2YUV)
    image_y = np.zeros(img_gray.shape[0:2], np.uint8)
    image_y[:, :] = img_gray[:, :, 0]

    image_blurred = cv._gaussian_blur(image_y, (3, 3), 0)
    img_edges = cv.Canny(image_blurred, 250, 250, aperture_size=3)
    lines = cv._hough_lines_p(img_edges, 1, math.pi / 180.0, 100, min_line_length=100, max_line_gap=20)
...
def rotate_image(mat, angle):
  ...
  rotation_mat = cv.get_rotation_matrix2_d(image_center, angle, 1.)
  ...
  rotated_mat = cv.warp_affine(mat, rotation_mat, (bound_w, bound_h), border_value=(255,255,255))
  return rotated_mat
...
def main():
    try:
        ...
        shadows_out = cv.copy_make_border(shadows_out, 2, 2, 2, 2, cv.BORDER_CONSTANT, value=[1, 0, 0])
